...
def plotStatsComparisonaAll(statsall: List[Dict[str, Any]], save_dir: str):
    ...
    for param_idx, param_label in enumerate(labels):
        ...
        axes[0].set_xticklabels(dataset_names, rotation=45)
        axes[0].legend()
        ...
        axes[1].set_xticklabels(dataset_names, rotation=45)
        axes[1].legend()
    ...
    if len(lc_interval_all) > 0 and len(tc_interval_all) > 0:
        for param_idx, param_label in enumerate(['Hor Pos', 'Alt']):
            ...
            for interval_idx, interval_name in enumerate(interval_labels):
                ...
                axes[interval_idx].set_xticklabels(dataset_names, rotation=45)
...
#函数入口
def bppaps(foldobjbase_lc: str = '/mnt/d/dockers/test/rts',
    ...
    for i in range(len(objMatchedPathsTC)):
        ...
        # refdata[:, 7:9] = (refdata[:, 7:9] - refdata[0, 7:9]) * r2m #非copy，不能再算，也无需再算
        ...
        save_dir = os.path.join(os.path.dirname(objMatchedPathsTC[i]),'detail_plots')
        os.makedirs(save_dir, exist_ok=True)
        ...
        try:
            ...
            compare_tc_lc_ref_rts_with_plots(
...
if __name__ == "__main__":
    # 示例调用（实际使用时请提供正确的路径）
    result = bppaps('/mnt/d/dockers/test/rts','/mnt/d/dockers/test/rts','/mnt/d/dockers/test/rts','msf_debug_state.csv')
    pass
