...
def compare_tc_lc_ref_rts():
    ...
    tcver2 = 'tmp'
    tcver2 = 'tmpLC_pre300-420'
    # tcver2 = 'tmpLC_pre300-420'
    lcver = 'LC_968e3ff7'
    lcver = 'tmpLC_pre300-420'
    lcver = 'tmpLC_pre90-210'
    
    rtsver = 'rts_2bf9a53'
    rtsver = 'rts'
    rtsver = 'rts_6304322df'
    basefold = '/mnt/d/dockers/test'  #e2e_chenshixiagu_2025一直有问题
    ...
    tcstemp = lcs
    lcs = utils.InterpState(tcstemp, tcstemp[:,0], tref)
    ...
    pkoffset = 20
    pi = 3 * (si - 1) + 1 + 1  # pi=3*(si-1)+1+i
    pki_idx = pi - 3 + pkoffset
    pkie_idx = pi - 3 + 2 + pkoffset
    
    plt.subplot(2, 1, 1)
    p3dtc = np.sqrt(np.sum(diff_datalc[:, pi_idx:pie_idx]**2, axis=1))  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(common_timelc - t0, p3dtc, "-o", linewidth=1, color='red', markersize=1, label='LC_error')
    p3dtc = np.sqrt(np.sum(lcs[:, pki_idx:pkie_idx]**2, axis=1))  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(lcs[:,0] - t0, p3dtc, "-o", linewidth=1, color='blue', markersize=1, label='LC_Pk')
    
    p3dtc = np.sqrt(np.sum(diff_datatc[:, pi_idx:pie_idx]**2, axis=1))  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(common_timetc - t0, p3dtc, "-o", linewidth=1, color='magenta', markersize=1, label='TC_error')
    p3dtc = np.sqrt(np.sum(tcs[:, pki_idx:pkie_idx]**2, axis=1))  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(tcs[:,0] - t0, p3dtc, "-o", linewidth=1, color='green', markersize=1, label='TC_Pk')
    ...
    plt.subplot(2, 1, 2)
    p3dtc = np.fabs(diff_datalc[:, pie_idx])  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(common_timelc - t0, p3dtc, "-o", linewidth=1, color='red', markersize=1, label='LC_error')
    p3dtc = lcs[:,pkie_idx]  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(lcs[:,0] - t0, p3dtc, "-o", linewidth=1, color='blue', markersize=1, label='LC_Pk')
    
    p3dtc = np.fabs(diff_datatc[:, pie_idx])  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(common_timetc - t0, p3dtc, "-o", linewidth=1, color='magenta', markersize=1, label='TC_error')
    p3dtc = tcs[:,pkie_idx]  # pi_idx-2:pie_idx-1 (Python切片)
    plt.plot(tcs[:,0] - t0, p3dtc, "-o", linewidth=1, color='green', markersize=1, label='TC_Pk')
    
    plt.plot(tcs[:,0] - t0, tcs[:, -1]/10, "-.", linewidth=1, color='black', markersize=1, label='TC_stat')
