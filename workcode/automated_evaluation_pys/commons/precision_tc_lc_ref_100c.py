...
def precision3d_tc_lc_ref_100c_():
    ...
    lcver = 'TC_8255e68a' #tcmsf_biased 打结版本
    lcver = 'TC_9b3f565'
    ...
    for si in range(1, 7):  # 1:3 对应Python中的range(1, 4)
        plt.figure(si)
        ...
        for i in range(1, 4):  # 1:3 对应Python中的range(1, 4)
            ...
            if si < 4:
                ...
                # 检查数据维度是否足够
                plt.plot(common_timelc - t0, diff_datalc[:, pi_idx], "-o", linewidth=1, color='red', markersize=1, label='diff_lc')
                plt.plot(common_timetc - t0, diff_datatc[:, pi_idx], "-o", linewidth=1, color='blue', markersize=1, label='diff_tc')
                plt.xlim(xranges)
