...
bool MapProcessor::UpdateStateByMap(const MapPosDataPtr map_data_ptr, StatePtr state_ptr, KfPtr<21, 3> kf_ptr) {
    ...
    Eigen::Vector3d dpos_rfu_          = local_trans.LLAtoEgoRfu(state_ll0_, map_ll0_, state_ptr->att);
...

    {
        ...
        Eigen::Vector3d dpos_rfu_mb_          = local_trans.LLAtoEgoRfu(veh_ll0_with_mb_, map_ll0_, state_ptr->att);
        double          dpos_mb_distance_     = dpos_rfu_mb_.norm();
        ...
        sdmap_str = fmt::format(                                     //
            ...
            map_data_ptr->lla.x() * 180.0 / M_PI,                    // map lat
            map_data_ptr->lla.y() * 180.0 / M_PI,                    // map lon
...
void MapProcessor::UpdateMapbiasSppMode(const Eigen::Vector3d &map_inno_enu, double dt, StatePtr state_ptr) {
    ...
    if (map_inno_enu.block<2, 1>(0, 0).norm() < min_thres_trig_corr_) {
        // 如果地图距离自车小于一定值，则不做处理
        return;
    }
    ...
    map_inno_enu_clamped.head<2>() =
        map_inno_enu.head<2>().array().abs().min(delta_bias_bound_.array()) *
