...
def two_version_cmp_bias_esti_state():
    """
    ...
    dataset = '20250429_huanlegu' #be  
    
    lcpath = os.path.join(basefold, dataset, lcver, 'bias_esti_state.csv')#new
    tcpath = os.path.join(basefoldtc, dataset, tcver, 'bias_esti_state.csv')#old
    ...
    # 读取数据
    gnssindex = list(range(0,11))
    
    dt = 0
    tcs = fileutils.readfullcsv(tcpath, gnssindex)
    lcs = fileutils.readfullcsv(lcpath, gnssindex)
...
if __name__ == "__main__":
    if 1:
        two_version_cmp_bias_esti_state()
    else:
        tcver = 'tc_ebec524'
        lcver = 'rts_2bf9a53'
